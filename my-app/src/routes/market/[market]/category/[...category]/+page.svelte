<script lang="ts">
  ...
  $: link =  `/market/supermarkets/${search_category ?? ''}`
    export let data: PageData

    $: ({ CategoryProducts } = data)
    $: category = $CategoryProducts.data.marketCollection.edges[0].node.categoryCollection.edges[0].node
...
<main class="w-full flex-1 px-8 flex-col justify-start items-start gap-8 inline-flex">
	<div class="w-full sticky top-0 bg-white pt-4">
		<h1 class="font-sans text-black text-5xl font-semibold font-['Inter'] leading-10">
				<a class=" text-blue-600" href={`/market/supermarkets/${search_category ?? ''}`}>
          {category?.name?.en}
				</a>
		</h1>
		<FilterPane />
	...
	<section class="w-full products">
		{#each category?.productCollection?.edges ?? [] as { node: product }}
			<Product {product} />
		{/each}
	<section />
</main>
