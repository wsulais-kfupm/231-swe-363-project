...
async function table(table){
    return new Promise(function (resolve) {
        client.query(`SELECT * FROM ${table};`,(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
...
async function priceforproduct(productname){
    return new Promise(function (resolve) {
        client.query(`SELECT Price,currency,variant_slug from priceforproduct where product_slug=${productname}`,(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
            if(err) console.log(err)
            console.log(res)
            resolve(res.rows)
        })
...
async function productVariant(product){
    return new Promise(function (resolve) {
        client.query(`SELECT * from Product_Variant where product_slug = "${product}"`,(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
            if(err) console.log(err)
            console.log(res)
            resolve(res.rows)
        })
...
async function variantPrices(product,variantName){
    return new Promise(function (resolve) {
        client.query(`SELECT price,currency from product_Pricing where listing_ID in (select listing_ID from product_listing where varid in (select Var_ID from Product_Variant where variant_slug ='${variantName}' AND product_slug="${product}" order by ));`,(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
            if(err) console.log(err)
            console.log(res)
            resolve(res.rows)
        })
...
async function addEndPoint(subscription){
    return new Promise(function (resolve) {
        ...
        client.query(`INSERT INTO webuser values($1,$2,$3,$4)`,[subscription.sub.endpoint,subscription.sub.keys.auth,subscription.sub.keys.p256dh,j],(/** @type {any} */ err,/** @type {any} */ res)=>{
            if(err) console.log(err)
            resolve(1)
        })
...
async function addNotification(cookieID,variant){
    return new Promise(function (resolve) {
        client.query(`INSERT INTO Notified values(${cookieID},${variant})`,(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
            if(err) console.log(err)
            console.log(res)
            resolve(res.rows)
        })
...
async function userNotifications(cookieID){
    return new Promise(function (resolve) {
        client.query(`SELECT Var_ID from Notified where Cookie_ID= $1`,[cookieID+''],(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
            if(err) console.log(err)
            resolve(res.rows)
        })
...
async function psuhAllNotifications(){
    client.query(`SELECT * from webuser `,(/** @type {any} */ err,/** @type {{ rows: any; }} */ res)=>{
...
async function pushNotification(subscription){
    userNotifications(subscription.endpoint).then((data)=>{
        for(let i=0;i<data.length;i++){
            client.query('SELECT price,v.variant_slug from product_Pricing a join product_listing l on a.listing_id=l.listing_id join product_variant v on l.varid = v.var_id where a.listing_ID in (select listing_ID from product_listing where varid = $1) order by price ',[data[i].var_id],(/** @type {any} */ err,/** @type {{ rows: any[]; }} */ res)=>{
