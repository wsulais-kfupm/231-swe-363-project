<script lang="ts">
    ...
    import type { ProductInfo } from '$houdini'
    ...
    $: info = fragment(product, graphql(`
	    fragment ProductInfo on Product {
	        ...
	        name,
					...
					productQuantityCollection(orderBy: {count: AscNullsFirst, quantity: AscNullsFirst}) {
						edges {
							node {
								quantity,
								count,
	...
	export let pname = '';
	export let bname = $info?.brand?.name;
	export let img = $info?.img ?? '';
	...
	$: price = $info?.productQuantityCollection?.edges[current]?.node?.productListingCollection?.edges[0]?.node?.priceDeltaCollection?.edges[0]?.node
</script>
	...
	<div class="product-info">
		...
		<div class="weights-container">
			<div class="weights">
				...
				{#each $info?.productQuantityCollection.edges ?? [] as { node: quantity }, i}
					<button class="nav-link {current === i ? 'selected' : ''}" on:click={() => (current = i)}>
						<span class="weight-text">{(quantity.count > 1 ? `${quantity.count}×` : "") + Intl.NumberFormat("en", { style: "unit", unit: quantity.unit }).format(quantity.quantity)}</span>
					</button>
				{/each}
			</div>
		</div>
